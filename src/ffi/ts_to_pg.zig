...
pub fn ts_to_postgres(allocator: std.mem.Allocator, typescript: []const u8) ![]const u8 {
    var output = std.ArrayList(u8).init(allocator);
    defer output.deinit();

    var lines = std.mem.split(u8, typescript, "\n");
...
//     const result = ts_to_postgres(std.heap.c_allocator, std.mem.span(input)) catch unreachable;
...
//
export fn ts_2_pg(input: [*:0]const u8) [*:0]u8 {
    const result = ts_to_postgres(std.heap.c_allocator, std.mem.span(input)) catch |err| {
